...
import sys
import logging
logging.basicConfig(filename='benchmark.log', level=logging.INFO, filemode="a", format="%(message)s")
...
def create_ff(inp, ff):
	...
	if trial == 999:
		logging.info("trials exceeded {trial}! will exit now.")
	return create
...
def gmx_bm(tpr: str, nsteps:int=20000):
	...
	for i, p in enumerate(parallels):
		...
		for j, c in enumerate([cpus[i]]*p):
			...
			torun += f"-notunepme -nb gpu -bonded gpu -pme gpu -resethway "
		...
		for line in out.stderr.split("\n"):
			if "Performance:" in line:
				...
				logging.info(f"{float(line.split()[1]):.04f} ns/day | " + commands[prints + 1])
				prints += 1
		logging.info("---        ---")
		logging.info(f"Total performance for {p} parallels: {np.sum(perfs[i]):.04f} ns/day\n")
